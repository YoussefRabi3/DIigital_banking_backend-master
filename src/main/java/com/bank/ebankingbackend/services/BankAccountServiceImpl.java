...
/*
    @Override
    public void saveDebit(String accountId,double amount, String description) throws BankAccountNotFoundException, BalanceNotSufficentException {
    ...
    @Override
    public List<BankAccountDTO> getAccountCustomer(Long accountId) {
        List<BankAccount> bankAccounts = bankAccountRepository.findBankAccountByCustomerId(accountId);
        List<BankAccountDTO> bankAccountDTOS = bankAccounts.stream().map(bankAccount -> {
            if (bankAccount instanceof SavingAccount) {
                SavingAccount savingAccount = (SavingAccount) bankAccount;
                return dtoMapper.fromSavingBankAccount(savingAccount);
